...
Automates GCP project setup tasks using a declarative `gcp_components.yaml`.
...
Features
--------
- Enables APIs, creates/ensures service accounts and keys, and applies IAM bindings.
- Writes a `setup_run_log.json` manifest describing actions performed.

Notes
-----
- Requires `gcloud` installed and appropriate permissions in the target project.
- Some long-running `gcloud` operations may require increasing the timeout in `gcp_config.yaml`.

---

Below is the repository `SETUP_GCP.md` content (authoritative guide):
